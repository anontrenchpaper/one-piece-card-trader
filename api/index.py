...
def search_card_by_number(card_input: str, one_piece_only: bool = True) -> List[Dict[str, Any]]:
    ...
    try:
        res = requests.post(TCGPLAYER_SEARCH_API, json=payload_term, headers=search_headers, timeout=10)
        if res.status_code == 200:
        ...
        if not raw_results:
            ...
            res_ex = requests.post(TCGPLAYER_SEARCH_API, json=payload_extracted, headers=search_headers, timeout=10)
